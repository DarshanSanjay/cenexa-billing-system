...
-- Both admin and staff can view products
...
-- Both admins can update products, and staff can update stock during billing
...
-- Bills RLS Policies:
-- Admins can view all bills; Staff can view all bills for receipt printing or bills created by them
CREATE POLICY "Admins and Staff can view bills"
  ON public.bills FOR SELECT
  TO authenticated
  USING (public.is_admin() OR created_by = auth.uid() OR true);

-- Both admins and staff can insert bills
CREATE POLICY "Authenticated users can create bills"
  ON public.bills FOR INSERT
  TO authenticated
  WITH CHECK (true);

-- Only admins can update or delete bills
