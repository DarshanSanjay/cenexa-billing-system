...
import { Camera, CheckCircle2, ClipboardPaste, Copy, RotateCcw, X } from 'lucide-react'
...
type Props = {
  open: boolean
  onClose: () => void
  onDetected: (payload: ScanPayload) => void
}
...
function validate(value: string, format: number | undefined): boolean {
  ...
  if (format === BarcodeFormat.EAN_13 || format === BarcodeFormat.UPC_A || format === BarcodeFormat.EAN_8) {
    const expected = format === BarcodeFormat.EAN_13 ? 13 : format === BarcodeFormat.UPC_A ? 12 : 8
...
export default function BarcodeScanner({ open, onClose, onDetected }: Props) {
  ...
  const startCamera = useCallback(async (deviceId?: string) => {
    ...
    try {
      ...
      const selected = deviceId || videoDevices[Math.min(cameraIndex, videoDevices.length - 1)]?.deviceId
      const reader = new BrowserMultiFormatReader(hints, { delayBetweenScanAttempts: 120, delayBetweenScanSuccess: 700 })
      readerRef.current = reader

      const controls = await reader.decodeFromVideoDevice(selected, videoRef.current!, (result, error) => {
        if (result) {
        ...
        if (error && !(error instanceof NotFoundException)) {
          // Decode misses are expected while the user is moving the barcode; do not surface them as failures.
        }
      })
  ...
  }, [cameraIndex, onDetected, open, stopCamera])
    ...
    <div className="barcodeOverlay" role="dialog" aria-modal="true" aria-label="Barcode scanner">
      <div className="barcodeModal">
        <div className="barcodeHeader">
          <div>
            <div className="barcodeKicker"><Camera size={13} /> LIVE SCANNER</div>
            <h3>Scan product barcode</h3>
            <p>Code 128 · EAN-13 · UPC-A · QR and more</p>
          </div>
          <button className="scannerClose" onClick={onClose} aria-label="Close scanner"><X size={19} /></button>
        </div>
        ...
        {cameras.length > 1 && (
          <div className="cameraPicker">
            ...
            <select value={cameraIndex} onChange={(event) => {
              ...
              {cameras.map((camera, index) => <option key={camera.deviceId || index} value={index}>{camera.label || `Camera ${index + 1}`}</option>)}
            </select>
          </div>
        )}

        <div className="manualScanner">
          <div className="manualTitle"><ClipboardPaste size={15} /><span>Camera unavailable?</span><small>Enter or paste the barcode</small></div>
          <div className="manualRow">
            <input value={manual} onChange={(event) => setManual(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') submitManual() }} placeholder="e.g. 8901234567890" inputMode="numeric" autoComplete="off" />
            <button className="primary" onClick={submitManual}>Add</button>
          </div>
        ...
        <div className="scannerFooter">
          ...
          <button onClick={() => { lastValueRef.current = ''; void startCamera() }}><RotateCcw size={14} /> Restart</button>
        </div>
      </div>
    </div>
  )
