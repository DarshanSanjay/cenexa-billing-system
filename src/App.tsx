...
import { BarChart3, Bell, Boxes, ChevronRight, CircleDollarSign, FileText, LayoutDashboard, LogOut, Plus, Printer, Search, Settings, ShoppingCart, Trash2, Users, X } from 'lucide-react'

type Product = { id:number; name:string; sku:string; category:string; price:number; stock:number; gst:number }
...
const seedProducts: Product[] = [
 {id:1,name:'Premium A4 Paper',sku:'PAP-001',category:'Stationery',price:320,stock:48,gst:18},
 {id:2,name:'Blue Ball Pen Pack',sku:'PEN-014',category:'Stationery',price:120,stock:85,gst:12},
 {id:3,name:'Wireless Mouse',sku:'TEC-031',category:'Electronics',price:650,stock:16,gst:18},
 {id:4,name:'USB-C Cable',sku:'TEC-044',category:'Electronics',price:399,stock:27,gst:18},
 {id:5,name:'Notebook A5',sku:'NB-005',category:'Stationery',price:85,stock:7,gst:12},
 {id:6,name:'Office Stapler',sku:'OFF-011',category:'Office',price:260,stock:11,gst:18},
]
...
const todaySales=(seedBills.reduce((s,b)=>s+b.total,0))
...
function App(){
 ...
 const filteredProducts=useMemo(()=>products.filter(p=>`${p.name} ${p.sku} ${p.category}`.toLowerCase().includes(query.toLowerCase())),[products,query])
...
function Billing(props:any){return <div className="billingLayout page"><section className="card productPane"><div className="sectionTop"><div><h3>Select Products</h3><p>Add items to the current invoice.</p></div><div className="search"><Search size={17}/><input value={props.query} onChange={e=>props.setQuery(e.target.value)} placeholder="Search product or SKU..."/></div></div><div className="productGrid">{props.products.map((p:Product)=><button className="productTile" key={p.id} onClick={()=>props.add(p)}><div className="tileIcon">{p.name.split(' ').map(x=>x[0]).slice(0,2).join('')}</div><div className="tileBody"><b>{p.name}</b><span>{p.sku} · {p.category}</span><strong>{money(p.price)}</strong></div><Plus className="tilePlus" size={18}/></button>)}</div></section>
 ...
 <div className="customerFields"><input value={props.customer} onChange={e=>props.setCustomer(e.target.value)} placeholder="Customer name"/><input value={props.phone} onChange={e=>props.setPhone(e.target.value)} placeholder="Phone number"/></div>
 <div className="cartItems">{props.cart.length?<>{props.cart.map((i:BillItem)=><div className="cartItem" key={i.id}><div><b>{i.name}</b><span>{money(i.price)} × {i.qty}</span></div><strong>{money(i.price*i.qty)}</strong><button onClick={()=>props.remove(i.id)}><Trash2 size={15}/></button></div>)}</>:<div className="empty"><ShoppingCart size={28}/><b>No items yet</b><span>Select products to start the bill.</span></div>}</div>
...
function Products({products,setProducts}:{products:Product[];setProducts:any}){const [open,setOpen]=useState(false);const [name,setName]=useState('');const [price,setPrice]=useState('');return <div className="page"><div className="card"><div className="sectionTop"><div><h3>Product Catalogue</h3><p>Manage prices, stock and GST rates.</p></div><button className="primary" onClick={()=>setOpen(true)}><Plus size={17}/> Add Product</button></div><div className="tableWrap"><table><thead><tr><th>Product</th><th>SKU</th><th>Category</th><th>Price</th><th>GST</th><th>Stock</th><th></th></tr></thead><tbody>{products.map(p=><tr key={p.id}><td><b>{p.name}</b></td><td>{p.sku}</td><td>{p.category}</td><td>{money(p.price)}</td><td>{p.gst}%</td><td><span className={p.stock<10?'stock low':'stock'}>{p.stock}</span></td><td><button className="iconBtn" onClick={()=>setProducts((x:Product[])=>x.filter(i=>i.id!==p.id))}><Trash2 size={16}/></button></td></tr>)}</tbody></table></div></div>{open&&<div className="modalOverlay"><div className="modal"><div className="modalHead"><div><h3>Add Product</h3><p>Create a new catalogue item.</p></div><button onClick={()=>setOpen(false)}><X/></button></div><input placeholder="Product name" value={name} onChange={e=>setName(e.target.value)}/><input placeholder="Price" type="number" value={price} onChange={e=>setPrice(e.target.value)}/><button className="primary full" disabled={!name||!price} onClick={()=>{setProducts((x:Product[])=>[...x,{id:Date.now(),name,sku:`CEN-${Date.now().toString().slice(-4)}`,category:'General',price:Number(price),stock:0,gst:18}]);setName('');setPrice('');setOpen(false)}}>Save Product</button></div></div>}</div>}
